...
class _DetailRecetteState extends State<DetailRecette> {
  ...
  void initState() {
    ...
    isLiked = widget.profile.hasLikedContent(recette);
    // firestoreService.getCurrentUserProfileRealTime().listen((Profile _profile) {
    //   setState(() {
    //     profile = _profile;
    //     if (profile != null) {
    //       isLiked = profile!.hasLikedContent(recette);
    //       print("profile Liked Recette: ${profile!.likedRecette.toString()}");
    //     }
    //   });
    // });
  ...
  void dispose() {
    ...
    _nomIngredientsController.forEach((controller) => controller.dispose());
    _quantiteController.forEach((controller) => controller.dispose());
    //profile = null; // set profile to null
    super.dispose();
  }
                ...
                child: IconButton(
                  onPressed: () async {
                    ...
                    if (isLiked!) {
                      ...
                      // recette.unlikeContent(profile!.idProfile);
                      ...
                      // recette.likeContent(profile!.idProfile);
                      // await widget.refreshAllRecette;
                    }
                  },
                  icon: isLiked
